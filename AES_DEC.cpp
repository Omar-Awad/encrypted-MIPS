...
sc_lv<8> AES_DEC::mult9(sc_uint<8> i){
sc_lv<8> LUT9[256] ={0x00,0x09,0x12,0x1b,0x24,0x2d,0x36,0x3f,0x48,0x41,0x5a,0x53,0x6c,0x65,0x7e,0x77,
...
sc_lv<8> AES_DEC::mult11(sc_uint<8> i){
sc_lv<8> LUT11[256] ={0x00,0x0b,0x16,0x1d,0x2c,0x27,0x3a,0x31,0x58,0x53,0x4e,0x45,0x74,0x7f,0x62,0x69,
...
sc_lv<8> AES_DEC::mult13(sc_uint<8> i){
sc_lv<8> LUT13[256] ={0x00,0x0d,0x1a,0x17,0x34,0x39,0x2e,0x23,0x68,0x65,0x72,0x7f,0x5c,0x51,0x46,0x4b,
...
sc_lv<8> AES_DEC::mult14(sc_uint<8> i){
sc_lv<8> LUT14[256] ={0x00,0x0e,0x1c,0x12,0x38,0x36,0x24,0x2a,0x70,0x7e,0x6c,0x62,0x48,0x46,0x54,0x5a,
...
void AES_DEC::invMixColumns(){
	sc_lv<8> tmp[4][4];
	for(int i=0; i<4;i++){
			tmp[0][i] = mult14(data[0][i])^mult11(data[1][i])^mult13(data[2][i])^mult9(data[3][i]);
			tmp[1][i] = mult9(data[0][i])^mult14(data[1][i])^mult11(data[2][i])^mult13(data[3][i]);
			tmp[2][i] = mult13(data[0][i])^mult9(data[1][i])^mult14(data[2][i])^mult11(data[3][i]);
			tmp[3][i] = mult11(data[0][i])^mult13(data[1][i])^mult9(data[2][i])^mult14(data[3][i]);
	}
